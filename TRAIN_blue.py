...
t1 = time.localtime()
Train_Image_Dir="Data_Zoo/train/" # Images and labels for training
Train_Label_Dir="Data_Zoo/label/"# Annotetion in png format for train images and validation images (assume the name of the images and annotation images are the same (but annotation is always png format))
UseValidationSet=True# do you want to use validation set in training
Valid_Image_Dir="Data_Zoo/valid/"# Validation images that will be used to evaluate training
Valid_Labels_Dir="Data_Zoo/label/"#  (the  Labels are in same folder as the training set)
...
Batch_Size= 16 # Number of files per training iteration
