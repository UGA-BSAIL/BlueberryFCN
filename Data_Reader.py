...
################################Initiate folders were files are and list of train images############################################################################
    def __init__(self, ImageDir,GTLabelDir="", BatchSize=1,Suffle=True):
        ...
        self.OrderedFiles += [each for each in os.listdir(self.Image_Dir) if each.endswith('.PNG') or each.endswith('.mat') or each.endswith('.TIF') or each.endswith('.GIF') or each.endswith('.png') or each.endswith('.jpg') or each.endswith('.tif') or each.endswith('.gif') ] # Get list of training images
...
#.............Read image and labels from files.........................................................
           Img = scipy.io.loadmat(self.Image_Dir + "/" + self.SFiles[self.itr])
...
#............Set Batch image size according to first image in the batch...................................................
           if f==0:
                ...
                Cry = np.int32(Cry)
                Crx = np.int32(Crx)
                Images = np.zeros([batch_size,Cry,Crx,87], dtype=np.float)
...
#           Img = misc.imresize(Img, [Sy,Sx], interp='bilinear')
           Img = transform.resize(Img,(Sy, Sx), preserve_range=True)
...
#           blue = Img[:,:,6:9]
#           
#           red = misc.imresize(red, [Sy,Sx], interp='bilinear')
#           green = misc.imresize(blue, [Sy,Sx], interp='bilinear')
#           blue = misc.imresize(blue, [Sy,Sx], interp='bilinear')
           
#           Img = np.dstack((red, green,blue))
#           Img =np.stack((Img, green), axis=2)
...
######################################Read next batch of images and labels with no augmentation######################################################################################################
    def ReadNextBatchClean(self): #Read image and labels without agumenting
        ...
        for f in range(batch_size):
##.............Read image and labels from files.........................................................
           Img = scipy.io.loadmat(self.Image_Dir + "/" + self.OrderedFiles[self.itr])
            #Img = misc.imread(self.Image_Dir + "/" + self.SFiles[self.itr])
           Img=Img['PCs']
...
#           Img9 = Img[:,:,24]
#           
#           Img = np.dstack((Img1, Img2,Img3,Img4,Img5,Img6,Img7,Img8,Img9))
#           PC1 = Img[:,:,91]
#           PC2 = Img[:,:,92]
##           PC3 = Img[:,:,93]
#           
...
#............Set Batch size according to first image...................................................
           if f==0:
                ...
                Images = np.zeros([batch_size,Sy,Sx,87], dtype=np.float)
